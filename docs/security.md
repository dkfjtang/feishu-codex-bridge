...
- 附件下载 adapter 当前只有脱敏契约和 transport-backed 包装；adapter 请求不包含附件 key、文件名、文件路径或内容，返回结果也只保留状态、原因、附件类型和审批 id。当前 SDK transport 尚未提供真实下载方法，默认实现不会调用飞书下载 API、不会创建临时文件、不会把附件内容交给 Codex。
