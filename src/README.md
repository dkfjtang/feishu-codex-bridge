...
- `feishu/event-handler.js` 已具备基础 OpenClaw 对齐护栏：app_id 校验、自回声过滤、message_id 去重、过期事件丢弃和按 chat 串行队列。
...
- `runtime/bridge-runtime.js`：私聊文本消息到 policy、thread store、Codex session、streamed events 和任务卡片的最小编排，并对运行中卡片更新做节流。
